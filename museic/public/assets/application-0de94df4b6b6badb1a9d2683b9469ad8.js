...
(function() {
  var randRange, tossSheets;

  $(function() {
    return null;
  });

  randRange = function(zero, nplus1) {
    var diff;
    diff = nplus1 - zero;
    return zero + Math.floor(Math.random() * diff);
  };

  tossSheets = function() {
    var cap, cred, dem, eikone, img, intro, mama, marq, outos, p, pcs, post, stit, tit, _i, _len, _results;
    mama = $($('.posts')[0]);
  ...
  tossSheets = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      _results.push(img.css({
        transform: "rotate(" + (randRange(-7, 7)) + "deg)"
      }));
    }
    return _results;
  };
